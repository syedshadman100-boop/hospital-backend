...
export class BillingService {
  ...
  async createInvoice(dto: CreateInvoiceDto, hospitalId: string) {
  ...
  async findAll(hospitalId: string, filters: InvoiceFilterDto) {
    const {
      patientId,
      status,
      dateFrom,
  ...
  async createPayment(dto: CreatePaymentDto, hospitalId: string) {
    const invoice = await this.prisma.invoice.findUnique({
    ...
    if (invoice.status === 'cancelled' || invoice.status === 'refunded') {
      throw new BadRequestException(`Cannot add payment to an invoice with status '${invoice.status}'`);
    }
  ...
  async refundPayment(paymentId: string, dto: RefundPaymentDto, hospitalId: string) {
    ...
    if (payment.invoice.hospitalId !== hospitalId) {
      throw new BadRequestException('Payment does not belong to this hospital');
    }
    ...
    if (dto.amount > payment.amount.toNumber() - refundedSoFar) {
      throw new BadRequestException(
        `Refund amount exceeds available refund (₹${payment.amount.toNumber() - refundedSoFar})`,
      );
    }
    ...
    if (newPaidAmount <= 0) invoiceStatus = 'refunded';
    else if (newPaidAmount >= invoiceTotal) invoiceStatus = 'paid';
